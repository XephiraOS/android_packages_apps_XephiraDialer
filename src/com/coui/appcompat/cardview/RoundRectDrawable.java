...
class RoundRectDrawable extends Drawable {
    ...
    private boolean isUseSmoothRoundRect() {
        if (this.mIsSupportSmoothRoundCorner && isWeightValid()) {
            return true;
        }
        return false;
    }
    ...
    public void draw(Canvas canvas) {
        ...
        if (isUseSmoothRoundRect()) {
            path = new Path();
            OplusPath oplusPath = new OplusPath(path);
            RectF rectF = this.mBoundsF;
            float f10 = this.mRadius;
            oplusPath.addSmoothRoundRect(rectF, f10, f10, this.mWeight, Path.Direction.CCW);
        } else {
            path = COUIRoundRectUtil.getInstance().getPath(this.mBoundsF, this.mRadius);
        }
    ...
    public void getOutline(Outline outline) {
        if (isUseSmoothRoundRect()) {
            new OplusOutline(outline).setSmoothRoundRect(this.mBoundsI, this.mRadius, this.mWeight);
        } else {
    ...
    public RoundRectDrawable(ColorStateList colorStateList, float f10, float f11) {
