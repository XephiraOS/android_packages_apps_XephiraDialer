...
public class COUIPageIndicator extends View implements COUIIPagerIndicator {
    ...
    public class AccessibilityHelper extends a {
        ...
        public void onPopulateNodeForVirtualView(int i10, d dVar) {
            ...
            dVar.l0(quantityString);
            dVar.K0(quantityString);
            ...
            dVar.c0(this.mExceptionVirtualBounds);
            dVar.d0(this.mExceptionVirtualBounds);
            dVar.a(16);
        }
    }
    ...
    public class PageIndicatorModel {
        ...
        private final androidx.dynamicanimation.animation.d<PageIndicatorModel> mCurrentPosition = new androidx.dynamicanimation.animation.d<PageIndicatorModel>("currentPosition") { // from class: com.coui.appcompat.indicator.COUIPageIndicator.PageIndicatorModel.1
        ...
        public PageIndicatorModel(View view) {
            ...
            float[] fArr2 = {0.0f, f10, f11, f11 - ((COUIPageIndicator.INDICATOR_DOT_GONE_LEVEL_SIZE - fArr[2]) / 2.0f)};
            float[] fArr3 = {COUIPageIndicator.this.mNormalDotSize / 2.0f, COUIPageIndicator.this.mMediumDotSize / 2.0f, COUIPageIndicator.this.mSmallDotSize / 2.0f, 0.0f};
            ...
            while (it.hasNext()) {
                ...
                if (this.mCurrentOffset == 0.0f || (indexOf != (i10 = this.mCurrentIndex) && indexOf - 1 != i10)) {
                    ...
                    if (f12 >= fArr[0] && f10 - f11 <= fArr[1]) {
                        COUILog.d(COUIPageIndicator.DEBUG, COUIPageIndicator.TAG, "drawDots: dot index = " + indexOf + " dot radius = " + next.mRadius + " dot location = (" + next.mCenterX + ", " + next.mCenterY + ") left = " + this.mVisibleBounds[0] + " right = " + this.mVisibleBounds[1]);
                        if (indexOf == this.mCurrentIndex) {
                            next.setFillColor(COUIPageIndicator.this.mTraceColor);
                        } else {
                            next.setFillColor(COUIPageIndicator.this.mDotColor);
                        }
                        next.onDraw(canvas, COUIPageIndicator.this.mDotPaint);
                    }
                }
            }
        }
        ...
        */
        ...
        private void updatePath() {
            ...
            if (this.mCurrentIndex < this.mDots.size() - 1) {
                ...
                COUILog.d(COUIPageIndicator.DEBUG, COUIPageIndicator.TAG, "updatePath: mCurrentOffset = " + this.mCurrentOffset + " dots size = " + this.mDots.size() + " startDot = (" + centerX + ", " + centerY + ", " + radius + ") endDot = (" + centerX2 + ", " + centerY2 + ", " + radius2 + ") colorFactor = " + colorFactor + " moveFactor = " + moveFactor + " mDepartOffset = " + f10 + " mPortOffset = " + f18 + ") control1 = (" + f15 + ", " + sqrt + ") control2 = (" + f19 + ", " + f20 + ") control3 = (" + f17 + ", " + sqrt2 + ") snapAngle = " + asin);
                this.mPath.reset();
                ...
                float f27 = f21 + f10;
                float f28 = f25 + f10;
                this.mPathUp.arcTo(f27, f24, f28, f26, COUIPageIndicator.ONE_AND_A_HALF_OF_PI_IN_DEGREE, asin, false);
                ...
                this.mPathUp.arcTo(f30, f31, f32, f33, COUIPageIndicator.ONE_AND_A_HALF_OF_PI_IN_DEGREE - asin, asin, false);
                ...
                this.mPathDown.arcTo(f30, f31, f32, f33, COUIPageIndicator.HALF_OF_PI_IN_DEGREE, asin, false);
                this.mPathDown.quadTo(f19, (centerY2 * 2.0f) - f20, f15, (centerY * 2.0f) - sqrt);
                this.mPathDown.arcTo(f27, f24, f28, f26, COUIPageIndicator.HALF_OF_PI_IN_DEGREE - asin, asin, false);
        ...
        public int getClickedDotIndex(float f10, float f11) {
            ...
            while (it.hasNext()) {
                ...
                if (COUIPageIndicator.this.isLayoutRtl()) {
                    abs = Math.abs(fArr[0] - (next.getBounds().centerX() - (next.getBounds().width() / 2.0f)));
                } else {
                    abs = Math.abs(fArr[0] - next.getBounds().centerX());
                }
                if (i10 == -1 || abs < f12) {
                    i10 = this.mDots.indexOf(next);
                    f12 = abs;
                }
            }
            return i10;
    ...
    public void refresh() {
        ...
        if ("attr".equals(resourceTypeName)) {
            typedArray = getContext().obtainStyledAttributes(null, m.f6822Z0, this.mStyle, 0);
        } else if ("style".equals(resourceTypeName)) {
            typedArray = getContext().obtainStyledAttributes(null, m.f6822Z0, 0, this.mStyle);
        }
        if (typedArray != null) {
            this.mTraceColor = typedArray.getColor(m.f6855g1, 0);
            this.mDotColor = typedArray.getColor(m.f6832b1, 0);
            typedArray.recycle();
        }
    ...
    public COUIPageIndicator(Context context, AttributeSet attributeSet, int i10, int i11) {
        ...
        this.mDownTime = 0L;
        this.mIndicatorDescriptionID = i.f6673a;
        ...
        if (attributeSet != null) {
            TypedArray obtainStyledAttributes = context.obtainStyledAttributes(attributeSet, m.f6822Z0, i10, i11);
            this.mTraceColor = obtainStyledAttributes.getColor(m.f6855g1, 0);
            this.mDotColor = obtainStyledAttributes.getColor(m.f6832b1, 0);
            this.mNormalDotSize = obtainStyledAttributes.getDimension(m.f6837c1, 0.0f);
            this.mMediumDotSize = obtainStyledAttributes.getDimension(m.f6842d1, 0.0f);
            this.mSmallDotSize = obtainStyledAttributes.getDimension(m.f6847e1, 0.0f);
            this.mDotsInterval = obtainStyledAttributes.getDimension(m.f6851f1, 0.0f);
            this.mIsClickable = obtainStyledAttributes.getBoolean(m.f6827a1, true);
